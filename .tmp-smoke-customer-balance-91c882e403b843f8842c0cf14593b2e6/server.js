...
async function createLocalCollections() {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object") {
      state = {
        ...
        customerBalances: Array.isArray(parsed.customerBalances) ? parsed.customerBalances : [],
  ...
  return {
    ...
    customerBalances: new LocalCollection("customerBalances", state, persist),
...
function serializeCustomerBalance(record, userMap = new Map()) {
  const createdBy = record.createdBy ? userMap.get(normalizeIdString(record.createdBy)) : null;
  const totalAmount = roundMoney(record.totalAmount || roundMoney(record.paidAmount || 0) + roundMoney(record.remainingAmount || 0));
  const paidAmount = roundMoney(record.paidAmount || 0);
  const remainingAmount = roundMoney(record.remainingAmount || 0);
  return {
    id: normalizeIdString(record._id),
    customerName: safeString(record.customerName, 120),
    date: safeString(record.date, 10),
    totalAmount,
    paidAmount,
    remainingAmount,
    paymentStatus: paymentStatusFromAmounts(totalAmount, paidAmount),
    note: record.note || "",
    source: "manual",
    createdAt: record.createdAt,
    updatedAt: record.updatedAt || record.createdAt,
...
function normalizeSearchTerm(value = "", maxLength = 120) {
...
function matchesSearchTerm(term, ...values) {
  ...
  return values.some((value) => safeString(value, 300).toLowerCase().includes(term));
}

function workerAccountStatusLabel(status) {
  if (status === "debt") {
    return "Qarzdor";
  }
  if (status === "paid") {
...
function buildWorkerAccountBalanceNote(item) {
  if (item.debtAmount > 0) {
    return `Avans va chiqimlar hali ${item.debtAmount} so'mga yopilmagan. Ishchi shu balansni qoplab ishlayapti.`;
  }
  if (item.payableAmount > 0) {
    return `Ishchiga ${item.payableAmount} so'm to'lanishi kerak.`;
  }
  if (item.paidAmount > 0) {
    return "Hisob-kitob yopilgan va to'lov tizimda qayd qilingan.";
  }
  if (item.advanceAmount > 0) {
    return "Avans yozilgan, lekin shu oy hali to'liq yopilmagan harakat yo'q.";
  }
  return "Balans ochiq qarzsiz yopilgan.";
}

function buildCustomerAccountBalanceNote(item) {
  if (item.remainingAmount > 0 && item.paidAmount > 0) {
    return `Mijoz qisman to'lagan, yana ${item.remainingAmount} so'm qolgan.`;
  }
  if (item.remainingAmount > 0) {
    return `Mijozdan ${item.remainingAmount} so'm debitor qarz qolgan.`;
  }
  if (item.totalAmount > 0) {
    return "Hisob to'liq yopilgan.";
  }
  return "Hali yakunlangan buyurtma yo'q.";
}

function summarizeWorkerAccounts(allItems = [], filteredItems = []) {
  return {
    totalWorkers: allItems.length,
    filteredCount: filteredItems.length,
    accruedAmount: roundMoney(allItems.reduce((sum, item) => sum + roundMoney(item.accruedAmount || 0), 0)),
    advanceAmount: roundMoney(allItems.reduce((sum, item) => sum + roundMoney(item.advanceAmount || 0), 0)),
    paidAmount: roundMoney(allItems.reduce((sum, item) => sum + roundMoney(item.paidAmount || 0), 0)),
    payableAmount: roundMoney(allItems.reduce((sum, item) => sum + roundMoney(item.payableAmount || 0), 0)),
    debtAmount: roundMoney(allItems.reduce((sum, item) => sum + roundMoney(item.debtAmount || 0), 0)),
    processedQuantity: allItems.reduce((sum, item) => sum + Math.max(0, Math.round(item.processedQuantity || 0)), 0),
    debtorCount: allItems.filter((item) => item.debtAmount > 0).length,
    payableCount: allItems.filter((item) => item.payableAmount > 0).length,
    settledCount: allItems.filter((item) => item.payableAmount <= 0 && item.debtAmount <= 0).length,
  };
}

function summarizeCustomerAccounts(allItems = [], filteredItems = []) {
  const totalAmount = roundMoney(allItems.reduce((sum, item) => sum + roundMoney(item.totalAmount || 0), 0));
  const paidAmount = roundMoney(allItems.reduce((sum, item) => sum + roundMoney(item.paidAmount || 0), 0));
  const remainingAmount = roundMoney(allItems.reduce((sum, item) => sum + roundMoney(item.remainingAmount || 0), 0));
  return {
    totalCustomers: allItems.length,
    filteredCount: filteredItems.length,
    totalOrders: allItems.reduce((sum, item) => sum + Math.max(0, Math.round(item.totalOrders || 0)), 0),
    totalAmount,
    paidAmount,
    remainingAmount,
    collectionRate: totalAmount > 0 ? roundMoney((paidAmount / totalAmount) * 100) : 0,
    debtorCount: allItems.filter((item) => item.remainingAmount > 0).length,
    partialCount: allItems.filter((item) => item.paidAmount > 0 && item.remainingAmount > 0).length,
    paidCount: allItems.filter((item) => item.totalAmount > 0 && item.remainingAmount <= 0).length,
    unpaidCount: allItems.filter((item) => item.paidAmount <= 0 && item.remainingAmount > 0).length,
  };
}

async function buildWorkerAccounts({ monthKey = toMonthKey(), status = "all", search = "" } = {}) {
  const users = await collections.users.find({ role: "worker", isActive: true }).sort({ createdAt: 1 }).toArray();
  const [snapshots, recentTransactionsRaw] = await Promise.all([
    buildPayrollSnapshots({ monthKey, users }),
    collections.workerTransactions.find({}).sort({ date: -1, createdAt: -1 }).limit(80).toArray(),
  ]);
  const searchTerm = normalizeSearchTerm(search);
  const transactionMetaByWorker = new Map();

  for (const transaction of recentTransactionsRaw) {
    const workerId = normalizeIdString(transaction.workerId);
    if (!workerId) {
      continue;
    }
    if (!transactionMetaByWorker.has(workerId)) {
      transactionMetaByWorker.set(workerId, {
        count: 0,
        lastDate: "",
      });
    }
    const entry = transactionMetaByWorker.get(workerId);
    entry.count += 1;
    if (String(transaction.date || "") > String(entry.lastDate || "")) {
      entry.lastDate = safeString(transaction.date, 10);
    }
  }

  const allItems = snapshots.map((item) => {
    const userId = item.user?.id || "";
    const transactionMeta = transactionMetaByWorker.get(userId) || { count: 0, lastDate: "" };
    const balanceAmount = item.debtAmount > 0 ? item.debtAmount : item.payableAmount;
    const accountStatus = item.debtAmount > 0
      ? "debt"
    ...
    return {
      ...
      accountStatus,
      accountStatusLabel: workerAccountStatusLabel(accountStatus),
      recentTransactionCount: transactionMeta.count || 0,
      lastTransactionDate: transactionMeta.lastDate || "",
      balanceNote: buildWorkerAccountBalanceNote(item),
    };
  });

  const items = allItems
    .filter((item) => filterWorkerAccountByStatus(item, status))
    .filter((item) => matchesSearchTerm(
      searchTerm,
      item.user?.fullName,
      item.user?.username,
      item.user?.assignedWorkType,
      item.user?.assignedWorkTypeLabel,
    ))
    .sort((left, right) => {
  ...
  const visibleWorkerIds = new Set(items.map((item) => item.user?.id).filter(Boolean));
  const recentTransactions = await enrichWorkerTransactions(
    recentTransactionsRaw
      .filter((item) => visibleWorkerIds.has(normalizeIdString(item.workerId)))
      .slice(0, 30),
  );
  ...
  return {
    monthKey,
    status,
    q: safeString(search, 120),
    summary: summarizeWorkerAccounts(allItems, items),
    items,
    recentTransactions,
  };
}

async function buildCustomerAccounts(status = "all", search = "") {
  const [orderFinanceAll, manualBalanceRaw] = await Promise.all([
    getDeliveryFinanceEntries(),
    collections.customerBalances.find({}).sort({ date: -1, createdAt: -1 }).toArray(),
  ]);
  const orderFinance = orderFinanceAll.filter((item) => item.status === "done");
  const grouped = new Map();
  const searchTerm = normalizeSearchTerm(search);
  const customerBalanceUserMap = await getUserMapByIds(
    [...new Set(manualBalanceRaw.map((item) => normalizeIdString(item.createdBy)).filter(Boolean))],
  );
  const manualBalances = manualBalanceRaw.map((item) => serializeCustomerBalance(item, customerBalanceUserMap));
  ...
  for (const balance of manualBalances) {
    const key = safeString(balance.customerName, 120) || "Nomsiz zakazchi";
    if (!grouped.has(key)) {
      grouped.set(key, {
        customerName: key,
        totalOrders: 0,
    ...
    const current = grouped.get(key);
    current.totalOrders += 1;
    current.totalAmount = roundMoney(current.totalAmount + balance.totalAmount);
    current.paidAmount = roundMoney(current.paidAmount + balance.paidAmount);
    current.remainingAmount = roundMoney(current.remainingAmount + balance.remainingAmount);
    current.lastOrderDate = String(balance.date) > String(current.lastOrderDate) ? balance.date : current.lastOrderDate;
    current.orders.push({
      taskId: "",
      manualBalanceId: balance.id,
      customerName: balance.customerName,
      quantity: 0,
      date: balance.date,
      status: "manual",
      unitPrice: 0,
      revenue: balance.totalAmount,
  ...
  const allAccounts = [...grouped.values()]
    .map((item) => ({
      ...item,
      paymentStatus: paymentStatusFromAmounts(item.totalAmount, item.paidAmount),
      paidPercent: item.totalAmount > 0 ? roundMoney((item.paidAmount / item.totalAmount) * 100) : 0,
      balanceNote: buildCustomerAccountBalanceNote(item),
      orders: item.orders
  ...
  const accounts = allAccounts
    ...
    .filter((item) => matchesSearchTerm(searchTerm, item.customerName))
    .sort((left, right) => {
  ...
  const recentPaymentsMapped = recentPayments.map((payment) => ({
  ...
  return {
    status,
    q: safeString(search, 120),
    summary: summarizeCustomerAccounts(allAccounts, accounts),
    accounts,
...
async function ensureIndexes() {
  await Promise.all([
    ...
    ensureCollectionIndex(collections.customerBalances, { customerName: 1, date: -1 }),
  ...
  if (!MongoClient) {
    ...
    try {
      ...
      collections = {
        ...
        customerBalances: database.collection("customerBalances"),
...
function validateCustomerBalanceInput(body) {
  const customerName = safeString(body.customerName, 120);
  const date = safeString(body.date, 10) || getYmd();
  const paidAmount = Math.max(0, roundMoney(body.paidAmount || 0));
  const remainingAmount = Math.max(0, roundMoney(body.remainingAmount || 0));
  const note = safeString(body.note, 300);
  const totalAmount = roundMoney(paidAmount + remainingAmount);

  if (!customerName) {
    return { error: "Zakazchi nomi majburiy." };
  }

  if (!date) {
    return { error: "Sana majburiy." };
  }

  if (totalAmount <= 0) {
    return { error: "Olingan yoki qolgan summa kiritilishi shart." };
  }
...
async function handleWorkerAccountsList(request, response, urlObject) {
  ...
  const q = safeString(urlObject.searchParams.get("q"), 120);
  sendJson(response, 200, await buildWorkerAccounts({ monthKey, status, search: q }));
}
...
async function handleCustomerAccountsList(request, response, urlObject) {
  ...
  const q = safeString(urlObject.searchParams.get("q"), 120);
  const data = await buildCustomerAccounts(status, q);
  sendJson(response, 200, data);
}

async function handleCustomerBalanceCreate(request, response) {
  const auth = await requireAuth(request, response, ["admin"]);
  if (!auth) {
    return;
  }
  ...
  const body = await readBody(request);
  const validation = validateCustomerBalanceInput(body);
  if (validation.error) {
    return sendJson(response, 400, { error: validation.error });
  }
  ...
  const result = await collections.customerBalances.insertOne(payload);
  const userMap = await getUserMapByIds([auth.user._id]);
  sendJson(response, 201, { ok: true, item: serializeCustomerBalance({ ...payload, _id: result.insertedId }, userMap) });
}

async function handleCustomerBalanceUpdate(request, response, id) {
  const auth = await requireAuth(request, response, ["admin"]);
  if (!auth) {
    return;
  }
  ...
  if (!ObjectId.isValid(id)) {
    return sendJson(response, 400, { error: "Qo'lda kiritilgan qoldiq ID noto'g'ri." });
  }

  const existing = await collections.customerBalances.findOne({ _id: new ObjectId(id) });
  if (!existing) {
    return sendJson(response, 404, { error: "Zakazchi qoldig'i topilmadi." });
  }
  ...
  await collections.customerBalances.updateOne({ _id: existing._id }, { $set: payload });
  const updated = await collections.customerBalances.findOne({ _id: existing._id });
  const userIds = [updated.createdBy, auth.user._id].filter(Boolean);
  const userMap = await getUserMapByIds(userIds);
  sendJson(response, 200, { ok: true, item: serializeCustomerBalance(updated, userMap) });
}

async function handleCustomerBalanceDelete(request, response, id) {
  const auth = await requireAuth(request, response, ["admin"]);
  if (!auth) {
    return;
  }
  ...
  const result = await collections.customerBalances.deleteOne({ _id: new ObjectId(id) });
  if (!result.deletedCount) {
    return sendJson(response, 404, { error: "Zakazchi qoldig'i topilmadi." });
  }
...
async function handleApi(request, response, urlObject) {
  ...
  if (pathname === "/api/customer-balances" && request.method === "POST") {
    return handleCustomerBalanceCreate(request, response);
  }

  if (pathname.startsWith("/api/customer-balances/") && request.method === "PATCH") {
    const id = pathname.split("/").pop();
    return handleCustomerBalanceUpdate(request, response, id);
  }

  if (pathname.startsWith("/api/customer-balances/") && request.method === "DELETE") {
    const id = pathname.split("/").pop();
    return handleCustomerBalanceDelete(request, response, id);
  }
